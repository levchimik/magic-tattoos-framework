...
; (gated on GetWaveformRegistrySize() == 0 — see _ensureWaveformsRegistered).
; Idempotent on the C++ side (insert_or_assign).
...
; calling SetActorPulseAndFadeBatch. Cheap (one mutex acquire + count).
...
; Internally wraps the Roster Set loop in BeginBatch/EndBatch so all entries
